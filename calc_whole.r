...
calcGenomeLevel = function(K,m,grid,epr,alphaT,Total,CASE=NULL,CONTROL=NULL,PC=NA,TEST = test,QT=NULL,nameEsseble=NULL,JJ=NA,PRC=NA){
   Esp = epr*K
      ...
      if (!(twoSolutions & pr>PP[1000000])){
         ...
         if (!is.na(alpha) & !is.na(beta) & ((alpha/beta*K)<=0.5)){
            ...
            if (abs(mean(kEV)-Esp)/(sd(kEV)/sqrt(5000))<3){
               ...
               a1=proc.time()
               result <- get_AproxQ2(alpha,beta,K,alphaT,Total,Obj,perm,CASE=CASE,CONTROL=CONTROL,PC=PC,TEST = TEST,QT=QT,nameEsseble=NULL,JJ=JJ,PRC) 
